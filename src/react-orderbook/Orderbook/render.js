...
export const renderInitial = (
  vizState: { curBook: Orderbook, canvasHeight: number, canvasWidth: number },
  canvas: any
) => {
...
export const renderUpdate = (
  ...
  let volumeDiff: number, fixedPrice, isBid;
  ...
  if (change.modification) {
    ...
    if (vizState.activePrices[fixedPrice]) {
      ...
      console.warn(
        `All orders removed at price level ${fixedPrice} but we had no volume level there before!`
      );
      volumeDiff = 0;
    }
    ...
    vizState.trades.push({
      timestamp,
      volume: change.newTrade.amountTraded,
      isBid,
      price: fixedPrice,
    });
  ...
  const curBandIndex = getBandIndex(vizState, price);

  let newPriceVolume =
    volumeDiff +
    parseFloat(vizState.activePrices[fixedPrice] ? vizState.activePrices[fixedPrice].volume : 0);
  if (newPriceVolume < 0) {
    newPriceVolume = 0;
  ...
  if (
    curBandIndex >= 0 &&
    curBandIndex < vizState.priceGranularity &&
    timestamp < vizState.maxTimestamp
  ) {
  ...
  if (activeBand.volume < 0) {
    activeBand.volume = (0).toFixed(vizState.pricePrecision);
  ...
  if (curBandIndex >= 0 && curBandIndex < vizState.priceGranularity) {
    ...
    if (newVolume > +vizState.maxVisibleBandVolume && !vizState.manualZoom) {
      ...
      vizState.scaleColor = chroma
        .scale(vizState.colorScheme)
        .mode('lch')
        .domain([0, newVolume]);
  ...
  if (!vizState.manualZoom && change.newTrade) {
    if (change.newTrade.price >= 0.995 * vizState.maxPrice) {
      ...
      vizState.minPrice = (vizState.minPrice * 0.997).toFixed(vizState.pricePrecision);
  ...
  if (timestamp > vizState.minTimestamp + 0.95 * timeRange && !vizState.manualZoom) {
    vizState.maxTimestamp += 0.2 * (vizState.maxTimestamp - vizState.minTimestamp);
 ...
 */
export const drawBands = (vizState, curTimestamp, canvas) => {
  ...
  const ctx = canvas.getContext('2d');
  vizState.activeBands.forEach((band: BandDef, i: number) => {
...
export const drawBand = (
  vizState,
  band: { startTimestamp: number, endTimestamp: number },
  index: number,
  ctx
) => {
  ...
  ctx.fillRect(
    Math.ceil(topLeftCoords.x),
    Math.floor(topLeftCoords.y),
    Math.ceil(bottomRightCoords.x - topLeftCoords.x),
    bottomRightCoords.y - topLeftCoords.y
  );
};
 ...
 */
const getBandColor = (band, maxVisibleVolume: string, scaleColor) => scaleColor(+band.volume).hex();

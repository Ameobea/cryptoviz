 ...
 */
export const histRender = (vizState, canvas, recalcMaxBandValues) => {
  ...
  vizState.priceLevelUpdates
    .filter(levelUpdate => levelUpdate.timestamp <= vizState.minTimestamp)
    .forEach(({ price, volume, isBid }) => {
  ...
  const initialBandValues = getInitialBandValues(
    vizState.minTimestamp,
    curPriceLevels,
    vizState.minPrice,
    vizState.maxPrice,
    vizState.priceGranularity,
  ...
  _.each(vizState.priceLevelUpdates, ({ price, volume, timestamp, isBid }) => {
    ...
    const volumeDiff = initialPriceLevels[price]
      ? +volume - +initialPriceLevels[price].volume
      : +volume;

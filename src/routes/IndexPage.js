...
class IndexPage extends React.Component {
  constructor(props) {
    ...
    let handleCurrencies = currencyDefinitions => {
      ...
      const zippedCurrencies = _.zipObject(
        activeSymbols,
        activeSymbols.map(symbol => currencyDefinitions[symbol])
      );
      delete zippedCurrencies['BTC'];
      delete zippedCurrencies['USDT'];

      this.setState({ currencies: zippedCurrencies });
      if (activeSymbols.includes('BTC_ETH')) {
        return 'BTC_ETH';
      } else if (activeSymbols.includes('BTC_XMR')) {
        return 'BTC_XMR';
      } else {
  ...
  handleTrades(tradeHistory) {
    ...
    this.setState({ minPrice: minRate * 0.995, maxPrice: maxRate * 1.005 });
  }

  // function for handling the result of the HTTP request for the initial orderbook
  handleBook(parsedRes) {
    const mapPrices = (isBid, pricePrecision) => ([price, volume]) => ({
      price,
      volume: volume.toFixed(pricePrecision),
      isBid,
    });

    const bids = parsedRes.bids.map(mapPrices(true, this.props.pricePrecision));
    const asks = parsedRes.asks.map(mapPrices(false, this.props.pricePrecision));
    const initialBook = _.concat(bids, asks);

    // insert the initial book into the component's state
    console.log('setting initial book');
    this.setState({ initialBook });
  }
    ...
    switch (msg[0]) {
      ...
      case this.currencyChannel: {
        ...
        if (seq === this.lastSeq + 1) {
          ...
          this.lastUpdate = msg;
          // console.log(msg[2]);
          msg[2].forEach(this.processBookUpdate);
          ...
          while (this.buffer[seq + 1]) {
            ...
            this.buffer[seq + 1][2].forEach(update => {
          ...
          if (this.lastUpdate && !_.isEqual(this.lastUpdate, msg)) {
            console.error(
              `Same sequence number but different messages: ${JSON.stringify(
                this.lastUpdate
              )} != ${JSON.stringify(msg)}`
            );
          }
          ...
          console.error(
            `sequence number ${seq} was less than we expected and we don't have a buffer for it`
          );
          ...
          if (this.lastSeq + 10 < seq) {
            ...
            console.log(
              `Received out-of-sequence message with seq ${seq} (expected ${this.lastSeq +
                1}); buffering it.`
            );
            this.buffer[seq] = msg;
          } else {
        ...
        if (msg[0] > 0 && msg[0] < 1000) {
          if (msg[2][0][0] == 'i') {
            ...
            if (orderbook.currencyPair != this.currency) {
              console.error(
                `Expected symbol ${this.currency} but received data for ${orderbook.currencyPair}`
              );
            } else {
              this.currencyChannel = msg[0];
              const mergedBook = {};
              Object.keys(orderbook.orderBook[0]).forEach(price => {
                mergedBook[price] = { volume: orderbook.orderBook[0][price], isBid: false };
              });
              Object.keys(orderbook.orderBook[1]).forEach(price => {
...
IndexPage.propTypes = { pricePrecision: PropTypes.number };

IndexPage.defaultProps = { pricePrecision: 8 };
